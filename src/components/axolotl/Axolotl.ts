export default class Axolotl {
    ...
    { 
        ...
        //@ts-ignore
        this.axl = scene.matter.add.sprite(x, y, 'axolotl', null, { label: 'axolotl', isStatic: true, circleRadius: 20 }).setDepth(1);  
        ...
        // Call events when clicking down
            ...
            this.axl.on('pointerdown', () => {
              if (proximityCheck == false) {
                ...
                let outsideRange = scene.add.text(this.axl.x - 25, this.axl.y - 15, 'Too far').setTint(0xff0000).setFontSize(8).setFontFamily('PressStart2P').setResolution(5);
            ...
            for (var i = 0; i < pairs.length; i++)
            {
              ...
              
              {
                if (bodyA.label === 'playerSensor' && bodyB.label === 'axolotl' ) 
                {
                  ...
                  })  // Make this a dynamic function that you can just pass params to and it will run this style of text
